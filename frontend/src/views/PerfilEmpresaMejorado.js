    ...
    try {
      ...
      if (Object.keys(userUpdateData).length > 0) {
        ...
        try {
          ...
          console.log('Usuario actualizado exitosamente:', userResponse.data);
          ...
          throw new Error(`Error al actualizar usuario: ${userError.response?.data?.message || userError.message}`);
        }
      }
      ...
      if (formData.razon_social !== originalData.razon_social) empresaFormData.append('razon_social', formData.razon_social);
      if (formData.cuit !== originalData.cuit) empresaFormData.append('cuit', formData.cuit);
      if (formData.telefono !== originalData.telefono) empresaFormData.append('telefono', formData.telefono);
      if (formData.direccion !== originalData.direccion) empresaFormData.append('direccion', formData.direccion);
      if (formData.descripcion !== originalData.descripcion) empresaFormData.append('descripcion', formData.descripcion);
      if (formData.sitio_web !== originalData.sitio_web) empresaFormData.append('sitio_web', formData.sitio_web);
      if (formData.linkedin_url !== originalData.linkedin_url) empresaFormData.append('linkedin_url', formData.linkedin_url);
      if (formData.sector !== originalData.sector) empresaFormData.append('sector', formData.sector);
      if (formData.empleados_cantidad !== originalData.empleados_cantidad) empresaFormData.append('empleados_cantidad', formData.empleados_cantidad);
      
      // Archivos
      if (formData.logo instanceof File) empresaFormData.append('logo', formData.logo);

      console.log('Empresa FormData entries:', Array.from(empresaFormData.entries()));

      // Solo hacer request de empresa si hay cambios
      if (empresaFormData.entries().next().done === false) {
        console.log('Actualizando datos de empresa...');
        const formDataEntries = Array.from(empresaFormData.entries());
        console.log('Enviando FormData:', formDataEntries);
        formDataEntries.forEach(([key, value]) => {
          console.log(`  ${key}: ${value}`);
        });
        try {
          const empresaResponse = await api.put(`/empresas/${formData.empresaId}/update-profile`, empresaFormData, {
            headers: { 'Content-Type': 'multipart/form-data' }
          });
          console.log('Respuesta del servidor:', empresaResponse.data);
          console.log('Empresa actualizada exitosamente:', empresaResponse.data);
        } catch (empresaError) {
          console.error('Error actualizando empresa:', empresaError);
          throw new Error(`Error al actualizar empresa: ${empresaError.response?.data?.message || empresaError.message}`);
        }
      }

      setSuccess(
        <div>
          <strong>¡Perfil empresarial actualizado exitosamente!</strong>
          <div className="mt-1 small">
            {Object.keys(userUpdateData).length > 0 && <div>• Datos de usuario actualizados</div>}
            {empresaFormData.entries().next().done === false && <div>• Información empresarial actualizada</div>}
            {formData.logo instanceof File && <div>• Nuevo logo guardado correctamente</div>}
          </div>
        </div>
      );
      setEditMode(false);
      ...
      if (err.message && err.message.includes('Error al actualizar')) {
        // Error personalizado que creamos arriba
        setError(err.message);
      } else if (err.response?.status === 419) {
        setError('Error de autenticación. Por favor, recargue la página e intente nuevamente.');
        ...
        setError('Error al guardar el perfil. Verifique su conexión e intente nuevamente.');
      }
    } finally {
      setSaving(false);
    }
